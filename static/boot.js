...
function applyBotName(){
  ...
  const msg=$('msg');
  if(msg) msg.placeholder=t('message_placeholder', name);
}

(async()=>{
  // Load send key preference
  let _bootSettings={};
  try{const s=await api('/api/settings');_bootSettings=s;window._sendKey=s.send_key||'enter';window._showTokenUsage=!!s.show_token_usage;window._showCliSessions=!!s.show_cli_sessions;window._soundEnabled=!!s.sound_enabled;window._notificationsEnabled=!!s.notifications_enabled;window._botName=s.bot_name||'Hermes';const _theme=s.theme||'dark';document.documentElement.dataset.theme=_theme;localStorage.setItem('hermes-theme',_theme);if(s.language&&typeof setLocale==='function'){setLocale(s.language);if(typeof applyLocaleToDOM==='function')applyLocaleToDOM();}if(s.font_size){var _s=parseInt(s.font_size)||14;document.documentElement.style.zoom=Math.round(_s/14*100)/100;localStorage.setItem('hermes-font-size',String(_s));}if(s.font_theme&&s.font_theme!=='default'){document.documentElement.dataset.fontTheme=s.font_theme;localStorage.setItem('hermes-font-theme',s.font_theme);}applyBotName();}catch(e){window._sendKey='enter';window._showTokenUsage=false;window._showCliSessions=false;window._soundEnabled=false;window._notificationsEnabled=false;window._botName='Hermes';_bootSettings={check_for_updates:false};if(typeof applyLocaleToDOM==='function')applyLocaleToDOM();applyBotName();}
